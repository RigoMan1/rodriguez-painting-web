...
export const VNestedSymbol: InjectionKey<NestedProvide> = Symbol.for('sui:nested');
  ...
  const nested: NestedProvide = {
    ...
    root: {
      ...
      register: (id, parentId, isGroup) => {
        if (parentId && id !== parentId) parents.value.set(id, parentId);

        if (isGroup) children.value.set(id, []);
      ...
      open: (id, value, event) => {
        ...
        if (newOpened) opened.value = newOpened;
      },
      ...
      activate: (id, value, event) => {
        ...
        if (newActivated) activated.value = newActivated;
      },
      children,
      parents,
    },
...
export const useNestedItem = (id: Ref<unknown>, isGroup: boolean) => {
  ...
  if (isGroup) provide(VNestedSymbol, item);
