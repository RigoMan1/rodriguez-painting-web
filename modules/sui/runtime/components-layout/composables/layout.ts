...
export const SUILayoutKey: InjectionKey<LayoutProvide> = Symbol.for('sui:layout');
...
export function createLayout(props: { overlaps?: string[]; fullHeight?: boolean }) {
  ...
  provide(SUILayoutKey, {
    register: (
      ...
      const instances = findChildrenWithProvide(SUILayoutItemKey, rootVm?.vnode);
      ...
      const layoutItemStyles = computed<CSSProperties>(() => {
        ...
        const styles = {
          ...
          position:
            absolute.value || rootZIndex.value !== ROOT_ZINDEX ? 'absolute' : 'fixed',
          ...(transitionsEnabled.value ? undefined : { transition: 'none' }),
        } as const;
