...
export function useLocationStrategies(props: StrategyProps, data: LocationStrategyData) {
  ...
  function updateLocation() {
    ...
    const viewport = scrollParents.reduce<Box>((box: Box | undefined, el) => {
      ...
      if (box) {
        return new Box({
          ...
          width:
            Math.min(box.right, scrollBox.right) - Math.max(box.left, scrollBox.left),
          height:
            Math.min(box.bottom, scrollBox.bottom) - Math.max(box.top, scrollBox.top),
        });
      }
      return scrollBox;
    }, undefined!);
    ...
    Object.assign(contentStyles.value, {
      ...
      minWidth: convertToUnit(
        axis === 'y' ? Math.min(minWidth.value, targetBox.width) : minWidth.value
      ),
