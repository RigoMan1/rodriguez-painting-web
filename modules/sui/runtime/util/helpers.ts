...
export const createPropsPicker = (makePropsFn: () => Record<string, any>) => {
  const propsKeys = Object.keys(makePropsFn());

  return (props: Record<string, any>) => pick(props, propsKeys);
};
...
export function ensureValidVNode(vnodes: VNodeArrayChildren): VNodeArrayChildren | null {
  return vnodes.some((child) => {
    ...
    return (
      child.type !== Fragment || ensureValidVNode(child.children as VNodeArrayChildren)
    );
  })
    ? vnodes
    : null;
...
export function isClickInsideElement(event: MouseEvent, targetDiv: HTMLElement) {
  ...
  return (
    mouseX >= divLeft && mouseX <= divRight && mouseY >= divTop && mouseY <= divBottom
  );
}
